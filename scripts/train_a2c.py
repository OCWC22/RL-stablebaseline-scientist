#!/usr/bin/env python3
"""
Training script for Advantage Actor-Critic (A2C) algorithm on CartPole-v1 environment.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train an A2C agent on CartPole-v1")
    parser.add_argument(
...
def main():
    ...
    model_name = f"a2c_cartpole_{timestamp}"
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        save_replay_buffer=False,  # A2C is on-policy, no replay buffer to save
    ...
    model = A2C(
        ...
        n_steps=5,                 # Number of steps to collect per environment before updating
        gamma=0.99,                # Discount factor
        gae_lambda=1.0,            # GAE lambda parameter (1.0 = standard advantage estimation)
        ent_coef=0.0,              # Entropy coefficient
        vf_coef=0.5,               # Value function coefficient
        max_grad_norm=0.5,         # Max gradient norm for gradient clipping
        rms_prop_eps=1e-5,         # RMSprop epsilon
        use_rms_prop=True,         # Use RMSprop optimizer (True for A2C)
        normalize_advantage=False,  # Don't normalize advantages (standard A2C)
